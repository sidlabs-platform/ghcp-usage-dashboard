...
export interface LicenseRunHistoryProps {
  ...
  onReportErrorChange: (error: string | null) => void;
}
...
export function LicenseRunHistory({
  enterpriseSlug,
  selectedRunId,
  onSelectRun,
  onReportChange,
  onReportErrorChange,
}: LicenseRunHistoryProps) {
  ...
  useEffect(() => {
    ...
    setDetailError(null);
    setFailedRunId(null);
    onReportErrorChange(null);
  }, [enterpriseSlug, onReportErrorChange]);
  ...
  const handleSelect = (runId: string) => {
    ...
    fetch(`/api/billing/license-reconciliation/runs/${encodeURIComponent(runId)}?enterprise=${encodeURIComponent(enterpriseSlug)}`, {
      ...
      .then(async (res) => {
        ...
        if (!res.ok) {
          const message = "Failed to load run report.";
          setDetailError(message);
          setFailedRunId(runId);
          onReportErrorChange(message);
          onReportChange(null);
          return;
        }
      ...
      {detailError && (
        <div role="alert" className="flex flex-wrap items-center justify-between gap-2 rounded-md border border-red-300 bg-red-50 p-3 text-sm text-red-800 dark:border-red-900 dark:bg-red-950/40 dark:text-red-300">
          ...
          {failedRunId && (
            <button
              type="button"
              onClick={() => handleSelect(failedRunId)}
