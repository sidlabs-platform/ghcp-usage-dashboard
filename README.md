...
- The `billing_usage_records` dedup index likewise gained `product` and `unit_type` (`idx_billing_usage_dedup_v2`). A single SKU can be billed in more than one unit on the same day — `copilot_ai_credit` emits both `ai-credits` and `token-units` rows — so without them `INSERT OR REPLACE` kept only the last one and silently discarded the other unit's usage. Widening the index is strictly more permissive: no rows are rewritten and no table rebuild occurs, so the dashboard keeps working immediately with **no re-sync required**. Note the fix is **forward-only** — rows already discarded under the narrower key are not restored, and `billing_sync_state` still marks those dates as synced. To recover them for a historical period, re-run a billing sync for that range; until then, affected past periods under-report the unit that lost the collision (in practice `token-units`, which appear only in 2026-03 for the reference dataset).
